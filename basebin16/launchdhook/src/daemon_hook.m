...
xpc_object_t (*xpc_dictionary_get_value_orig)(xpc_object_t xdict,
                                              const char *key);
xpc_object_t xpc_dictionary_get_value_hook(xpc_object_t xdict,
                                           const char *key) {
    xpc_object_t orgValue = xpc_dictionary_get_value_orig(xdict, key);
    if (!strcmp(key, "LaunchDaemons")) {
        addLaunchDaemon(
            orgValue, prebootPath(@"basebin/LaunchDaemons/com.xia0o0o0o.jailbreakd.plist")
                          .fileSystemRepresentation);
        for (NSString *daemonPlistName in [[NSFileManager defaultManager]
                 contentsOfDirectoryAtPath:prebootPath(@"Library/LaunchDaemons")
                                     error:nil]) {
            if ([daemonPlistName.pathExtension isEqualToString:@"plist"]) {
                addLaunchDaemon(orgValue,
                                [prebootPath(@"Library/LaunchDaemons")
                                    stringByAppendingPathComponent:daemonPlistName]
                                    .fileSystemRepresentation);
            }
        }
    } else if (!strcmp(key, "Paths")) {
        xpc_array_set_string(
            orgValue, XPC_ARRAY_APPEND,
            prebootPath(@"basebin/LaunchDaemons").fileSystemRepresentation);
        xpc_array_set_string(
            orgValue, XPC_ARRAY_APPEND,
            prebootPath(@"Library/LaunchDaemons").fileSystemRepresentation);
    }
    return orgValue;
}
...
void initDaemonHooks(void) {
    MSHookFunction(&xpc_dictionary_get_value,
                   ...
                   (void **)&xpc_dictionary_get_value_orig);
}