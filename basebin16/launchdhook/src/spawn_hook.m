...
int posix_spawn_orig_wrapper(
    ...
    // crashreporter_pause();	//XXX NOT IMPLEMENTED
    int r = orig(pid, path, file_actions, attrp, argv, envp);
    // crashreporter_resume();	//XXX NOT IMPLEMENTED
...
int posix_spawn_hook(pid_t *restrict pid, const char *restrict path,
    ...
    if (path) {
        ...
        if (!strcmp(path, executablePath)) {
            ...
            // This has already been done in envp so we only need to call the regular
            // posix_spawn

            // But before, we want to pass the primitives to boomerang
            //   boomerang_userspaceRebootIncoming();	//XXX NOT IMPLEMENTED
            ...
            return posix_spawn_orig_wrapper(pid, path, file_actions, attrp, argv,
                                            envp);
        }
    }
    ...
    /*if (path) {
          ...
          if (!strcmp(path, "/usr/libexec/xpcproxy")) {
                  ...
                  size_t blacklistCount = sizeof(tmpBlacklist) /
  sizeof(tmpBlacklist[0]); for (size_t i = 0; i < blacklistCount; i++)
                  {
                          if (!strcmp(tmpBlacklist[i], firstArg)) {
                                  FILE *f = fopen("/var/mobile/launch_log.txt",
  "a"); fprintf(f, "blocked injection %s\n", firstArg); fclose(f); int
  (*orig)(pid_t *restrict, const char *restrict, const
  posix_spawn_file_actions_t *restrict, const posix_spawnattr_t *restrict, char
  *const[restrict], char *const[restrict]) = posix_spawn_orig; return orig(pid,
