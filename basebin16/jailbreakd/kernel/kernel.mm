...
uint64_t kalloc(size_t ksize) {
    ...
    JBLogDebug("[+] kernel_info.fake_userclient: 0x%llx", kernel_info.fake_userclient);
    ...
    JBLogDebug("[+] .container_init   : 0x%llx", kernel_info.kernel_functions.container_init);
    JBLogDebug("[+] .container_init- s: 0x%llx", kernel_info.kernel_functions.container_init  - kernel_info.kslide);
    JBLogDebug("[+] .container_init+ S: 0x%llx", kernel_info.kernel_functions.container_init + kernel_info.kslide);

    
    
    JBLogDebug("[+] .addr_proc_set_ucred: 0x%llx", kernel_info.kernel_functions.addr_proc_set_ucred);
    JBLogDebug("[+] .addr_proc_set_ucred- s: 0x%llx", kernel_info.kernel_functions.addr_proc_set_ucred  - kernel_info.kslide);
    JBLogDebug("[+] .addr_proc_set_ucred+ S: 0x%llx", kernel_info.kernel_functions.addr_proc_set_ucred + kernel_info.kslide);

    
    JBLogDebug("[+] .kcall_gadget: 0x%llx", kernel_info.kernel_functions.kcall_gadget );
    JBLogDebug("[+] .kcall_gadget- s: 0x%llx", kernel_info.kernel_functions.kcall_gadget  - kernel_info.kslide);
    JBLogDebug("[+] .kcall_gadget+ S: 0x%llx", kernel_info.kernel_functions.kcall_gadget + kernel_info.kslide);
    JBLogDebug("[+] .kread_gadget: 0x%llx", kernel_info.kernel_functions.kread_gadget);
    JBLogDebug("[+] .kwrite_gadget: 0x%llx", kernel_info.kernel_functions.kwrite_gadget);
    JBLogDebug("[+] .proc_updatecsflags: 0x%llx", kernel_info.kernel_functions.proc_updatecsflags);
    JBLogDebug("[+] .proc_updatecsflags- s: 0x%llx", kernel_info.kernel_functions.proc_updatecsflags  - kernel_info.kslide);
    JBLogDebug("[+] .proc_updatecsflags+ S: 0x%llx", kernel_info.kernel_functions.proc_updatecsflags + kernel_info.kslide);

    JBLogDebug("[+] off_empty_kdata_page        : 0x%llx", off_empty_kdata_page);
    JBLogDebug("[+] off_empty_kdata_page + slide: 0x%llx", off_empty_kdata_page + kernel_info.kslide);
    JBLogDebug("[+] off_empty_kdata_page - slide: 0x%llx", off_empty_kdata_page - kernel_info.kslide);
    JBLogDebug("[+] kbase : 0x%llx", kernel_info.kbase);
    JBLogDebug("[+] kbase + slide: 0x%llx", kernel_info.kbase + kernel_info.kslide);
    JBLogDebug("[+] kbase - slide: 0x%llx", kernel_info.kbase - kernel_info.kslide);
    JBLogDebug("[+] kproc : 0x%x", kernel_info.kproc);
    JBLogDebug("[+] kproc + slide: 0x%llx", kernel_info.kproc + kernel_info.kslide);
    JBLogDebug("[+] kproc - slide: 0x%llx", kernel_info.kproc - kernel_info.kslide);
    JBLogDebug("[+] self_proc : 0x%x", kernel_info.self_proc);
    JBLogDebug("[+] self_proc + slide: 0x%llx", kernel_info.self_proc + kernel_info.kslide);
    JBLogDebug("[+] self_proc - slide: 0x%llx", kernel_info.self_proc - kernel_info.kslide);
    JBLogDebug("[+] fake_userclient_vtable : 0x%x", kernel_info.fake_userclient_vtable);
    JBLogDebug("[+] fake_userclient_vtable + slide: 0x%llx", kernel_info.fake_userclient_vtable + kernel_info.kslide);
    JBLogDebug("[+] fake_userclient_vtable - slide: 0x%llx", kernel_info.fake_userclient_vtable - kernel_info.kslide);
    JBLogDebug("[+] pmap_image4_trust_caches : 0x%x", kernel_info.pmap_image4_trust_caches);
    JBLogDebug("[+] pmap_image4_trust_caches + slide: 0x%llx", kernel_info.pmap_image4_trust_caches + kernel_info.kslide);
    JBLogDebug("[+] pmap_image4_trust_caches - slide: 0x%llx", kernel_info.pmap_image4_trust_caches - kernel_info.kslide);
