...
typedef struct CF_BRIDGED_TYPE(
    id) __SecCode const *SecStaticCodeRef; /* code on disk */

typedef CF_OPTIONS(uint32_t, SecCSFlags) {
    kSecCSDefaultFlags = 0, /* no particular flags (default behavior) */

    kSecCSConsiderExpiration = 1U
                               << 31, /* consider expired certificates invalid */
    kSecCSEnforceRevocationChecks =
        1 << 30,                     /* force revocation checks regardless of preference settings */
    kSecCSNoNetworkAccess = 1 << 29, /* do not use the network, cancels
                                      "kSecCSEnforceRevocationChecks"  */
    kSecCSReportProgress =
        1 << 28, /* make progress report call-backs when configured */
    kSecCSCheckTrustedAnchors =
        1 << 27,                /* build certificate chain to system trust anchors, not to any
                  self-signed certificate */
    kSecCSQuickCheck = 1 << 26, /* (internal) */
    kSecCSApplyEmbeddedPolicy =
        1 << 25, /* Apply Embedded (iPhone) policy regardless of the platform
                  we're running on */
};
...
OSStatus SecCodeCopySigningInformation(SecStaticCodeRef code, SecCSFlags flags,
                                       CFDictionaryRef *__nonnull
                                           CF_RETURNS_RETAINED information);
...
#else
typedef struct __SecCodeSigner *
    SecCodeSignerRef SPI_AVAILABLE(macos(10.5), ios(15.0), macCatalyst(13.0));
#endif
...
#ifdef BRIDGED_SECCODESIGNER
OSStatus
SecCodeSignerCreate(CFDictionaryRef parameters, SecCSFlags flags,
...
int resignFile(NSString *filePath, bool preserveMetadata) {
    ...
    parameters[(__bridge NSString *)kSecCodeSignerIdentity] =
    ...
    status = SecCodeSignerCreate((__bridge CFDictionaryRef)parameters,
                                 kSecCSDefaultFlags, &signerRef);
    if (status == 0) {
        SecStaticCodeRef code;
        status = SecStaticCodeCreateWithPathAndAttributes(
            (__bridge CFURLRef)[NSURL fileURLWithPath:filePath], kSecCSDefaultFlags,
            NULL, &code);
        if (status == 0) {
            status = SecCodeSignerAddSignatureWithErrors(signerRef, code,
                                                         kSecCSDefaultFlags, NULL);
            if (status == 0) {
                CFDictionaryRef newSigningInformation;
                // Difference from codesign: added kSecCSSigningInformation,
                ...
                if (status == 0) {
                    NSLog(@"[jailbreakd] SecCodeCopySigningInformation succeeded: %s",
                          ((__bridge NSDictionary *)newSigningInformation)
                              .description.UTF8String);
                    retval = 0;
                    CFRelease(newSigningInformation);
                } else {
                    NSLog(@"[jailbreakd] SecCodeCopySigningInformation failed: %s (%d)",
                          ((__bridge NSString *)SecCopyErrorMessageString(status, NULL))
                              .UTF8String,
                          status);
                    retval = 203;
                }
            ...
            NSLog(@"[jailbreakd] SecStaticCodeCreateWithPathAndAttributes failed: %s "
                  @"(%d)",
                  ((__bridge NSString *)SecCopyErrorMessageString(status, NULL))
                      .UTF8String,
                  status);
        ...
        NSLog(@"[jailbreakd] SecCodeSignerCreate failed: %s (%d)",
              ((__bridge NSString *)SecCopyErrorMessageString(status, NULL))
                  .UTF8String,
              status);
        retval = 201;
