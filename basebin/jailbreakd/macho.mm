...
int64_t machoFindArch(FILE *machoFile, uint32_t subtypeToSearch) {
    ...
    machoEnumerateArchs(machoFile, ^(struct fat_arch *arch,
                                     uint32_t archMetadataOffset,
                                     uint32_t archOffset, BOOL *stop) {
      struct mach_header_64 mh;
      fseek(machoFile, archOffset, SEEK_SET);
      fread(&mh, sizeof(mh), 1, machoFile);
      uint32_t maskedSubtype =
          OSSwapLittleToHostInt32(mh.cpusubtype) & ~0x80000000;
...
void machoEnumerateArchs(FILE *machoFile,
                         void (^archEnumBlock)(struct fat_arch *arch,
                                               uint32_t archMetadataOffset,
                                               uint32_t archOffset,
                                               BOOL *stop)) {
    struct mach_header_64 mh;
    ...
    if (mh.magic == FAT_MAGIC || mh.magic == FAT_CIGAM) {
        ...
        for (int i = 0; i < OSSwapBigToHostInt32(fh.nfat_arch); i++) {
            ...
            BOOL stop = NO;
            archEnumBlock(&fatArch, archMetadataOffset,
                          OSSwapBigToHostInt32(fatArch.offset), &stop);
            if (stop)
                break;
        }
...
void machoGetInfo(FILE *candidateFile, bool *isMachoOut, bool *isLibraryOut) {
    ...
    bool isMacho = mh.magic == MH_MAGIC_64 || mh.magic == MH_CIGAM_64 ||
                   mh.magic == FAT_MAGIC || mh.magic == FAT_CIGAM;
    bool isLibrary = NO;
    if (isMacho && isLibraryOut) {
        __block int32_t anyArchOffset = 0;
        machoEnumerateArchs(candidateFile,
                            ^(struct fat_arch *arch, uint32_t archMetadataOffset,
                              uint32_t archOffset, BOOL *stop) {
        ...
        isLibrary = OSSwapLittleToHostInt32(mh.filetype) == MH_DYLIB ||
                    OSSwapLittleToHostInt32(mh.filetype) == MH_DYLIB_STUB;
    }

    if (isMachoOut)
        *isMachoOut = isMacho;
    if (isLibraryOut)
        *isLibraryOut = isLibrary;
}

void machoEnumerateLoadCommands(FILE *machoFile, uint32_t archOffset,
                                void (^enumerateBlock)(struct load_command cmd,
                                                       uint32_t cmdOffset)) {
    struct mach_header_64 mh;
    fseek(machoFile, archOffset, SEEK_SET);
    fread(&mh, sizeof(mh), 1, machoFile);
    ...
    uint32_t offset = 0;
    NSLog(@"[jailbreakd] [machoEnumerateLoadCommands] About to enumerate over %u "
          @"load "
           "commands (total size: 0x%X)",
          nCmds, sizeOfCmds);
    ...
    NSLog(@"[jailbreakd] [machoEnumerateLoadCommands] Finished enumerating over "
          @"%u load "
           "commands (total size: 0x%X)",
          nCmds, sizeOfCmds);
}

void machoFindCSData(FILE *machoFile, uint32_t archOffset, uint32_t *outOffset,
                     uint32_t *outSize) {
    machoEnumerateLoadCommands(
        machoFile, archOffset, ^(struct load_command cmd, uint32_t cmdOffset) {
          if (OSSwapLittleToHostInt32(cmd.cmd) == LC_CODE_SIGNATURE) {
              ...
              if (outOffset)
                  *outOffset =
                      archOffset + OSSwapLittleToHostInt32(CSCommand.dataoff);
              if (outSize)
                  *outSize = archOffset + OSSwapLittleToHostInt32(CSCommand.datasize);
          }
        });
}

void machoCSDataEnumerateBlobs(
    FILE *machoFile, uint32_t CSDataStart, uint32_t CSDataSize,
    void (^enumerateBlock)(struct CSBlob blobDescriptor,
                           uint32_t blobDescriptorOffset, BOOL *stop)) {
    ...
    if ((CSDataStart + blobLength) > (CSDataStart + CSDataSize))
        return;
    if ((sizeof(struct CSSuperBlob) + blobCount * sizeof(struct CSBlob)) >
        blobLength)
        return;

    for (int i = 0; i < blobCount; i++) {
        uint32_t blobDescriptorOffset =
            CSDataStart + sizeof(struct CSSuperBlob) + (i * sizeof(struct CSBlob));
...
bool machoCSDataIsAdHocSigned(FILE *machoFile, uint32_t CSDataStart,
                              uint32_t CSDataSize) {
    __block bool blobWrapperFound = false;

    machoCSDataEnumerateBlobs(
        machoFile, CSDataStart, CSDataSize,
        ^(struct CSBlob blobDescriptor, uint32_t blobDescriptorOffset,
          BOOL *stop) {
          ...
          if (blobType == CSSLOT_SIGNATURESLOT) {
              ...
              if (OSSwapBigToHostInt32(blobWrapper.magic) ==
                  CS_MAGIC_BLOB_WRAPPER) {
...
unsigned CSCodeDirectoryRank(CS_CodeDirectory *cd) {
    ...
    for (unsigned i = 0; i < sizeof(rankedHashTypes) / sizeof(rankedHashTypes[0]);
         i++) {
...
NSData *codeDirectoryCalculateCDHash(CS_CodeDirectory *cd, void *data,
                                     size_t size) {
...
NSData *machoCSDataCalculateCDHash(FILE *machoFile, uint32_t CSDataStart,
                                   uint32_t CSDataSize) {
    ...
    machoCSDataEnumerateBlobs(
        ...
        ^(struct CSBlob blobDescriptor, uint32_t blobDescriptorOffset,
          ...
          if (blobType == CSSLOT_CODEDIRECTORY ||
              ((CSSLOT_ALTERNATE_CODEDIRECTORIES <= blobType &&
                blobType < CSSLOT_ALTERNATE_CODEDIRECTORY_LIMIT))) {
              uint32_t blobDataOffset = OSSwapBigToHostInt32(blobDescriptor.offset);
    ...
    uint8_t *cdData = (uint8_t *)malloc(cdDataLength);
    if (!cdData)
        return nil;
...
NSString *processRpaths(NSString *path, NSString *tokenName, NSArray *rpaths) {
    ...
    if ([path containsString:tokenName]) {
        for (NSString *rpath in rpaths) {
            NSString *testPath = [path stringByReplacingOccurrencesOfString:tokenName
                                                                 withString:rpath];
...
NSString *resolveLoadPath(NSString *loadPath, NSString *machoPath,
                          NSString *sourceExecutablePath, NSArray *rpaths) {
    if (!loadPath || !machoPath)
        return nil;
    ...
    processedPath = [processedPath
        stringByReplacingOccurrencesOfString:@"@executable_path"
                                  withString:
                                      [sourceExecutablePath
                                          stringByDeletingLastPathComponent]];
    processedPath = [processedPath
        stringByReplacingOccurrencesOfString:@"@loader_path"
                                  withString:
                                      [machoPath
                                          stringByDeletingLastPathComponent]];
...
void _machoEnumerateDependencies(
    FILE *machoFile, uint32_t archOffset, NSString *machoPath,
    NSString *sourceExecutablePath, NSMutableSet *enumeratedCache,
    void (^enumerateBlock)(NSString *dependencyPath)) {
    if (!enumeratedCache)
        enumeratedCache = [NSMutableSet new];
    ...
    machoEnumerateLoadCommands(
        machoFile, archOffset, ^(struct load_command cmd, uint32_t cmdOffset) {
          if (OSSwapLittleToHostInt32(cmd.cmd) == LC_RPATH) {
              ...
              size_t stringLength = OSSwapLittleToHostInt32(rpathCommand.cmdsize) -
                                    sizeof(rpathCommand);
              char *rpathC = (char *)malloc(stringLength);
              fseek(machoFile,
                    cmdOffset + OSSwapLittleToHostInt32(rpathCommand.path.offset),
                    SEEK_SET);
              fread(rpathC, stringLength, 1, machoFile);
              NSString *rpath = [NSString stringWithUTF8String:rpathC];
              free(rpathC);

              NSString *resolvedRpath =
                  resolveLoadPath(rpath, machoPath, sourceExecutablePath, nil);
              if (resolvedRpath) {
    ...
    machoEnumerateLoadCommands(
        machoFile, archOffset, ^(struct load_command cmd, uint32_t cmdOffset) {
          ...
          if (cmdId == LC_LOAD_DYLIB || cmdId == LC_LOAD_WEAK_DYLIB ||
              cmdId == LC_REEXPORT_DYLIB) {
              struct dylib_command dylibCommand;
              fseek(machoFile, cmdOffset, SEEK_SET);
              fread(&dylibCommand, sizeof(dylibCommand), 1, machoFile);
              size_t stringLength = OSSwapLittleToHostInt32(dylibCommand.cmdsize) -
                                    sizeof(dylibCommand);
              char *imagePathC = (char *)malloc(stringLength);
              fseek(machoFile,
                    cmdOffset +
                        OSSwapLittleToHostInt32(dylibCommand.dylib.name.offset),
                    SEEK_SET);
              fread(imagePathC, stringLength, 1, machoFile);
              NSString *imagePath = [NSString stringWithUTF8String:imagePathC];
              free(imagePathC);

              BOOL inDSC = _dyld_shared_cache_contains_path(
                  imagePath.fileSystemRepresentation);
              if (!inDSC) {
                  NSString *resolvedPath = resolveLoadPath(
                      imagePath, machoPath, sourceExecutablePath, rpaths);
                  ...
                  if (![enumeratedCache containsObject:resolvedPath] &&
                      ...
                      NSLog(@"[jailbreakd] [_machoEnumerateDependencies] Found "
                            @"depdendency %s, recursively enumerating over it...",
                            resolvedPath.UTF8String);
                      FILE *nextFile =
                          fopen(resolvedPath.fileSystemRepresentation, "rb");
                      if (nextFile) {
                          ...
                          if (nextFileIsMacho) {
                              ...
                              if (nextBestArchCandidate >= 0) {
                                  _machoEnumerateDependencies(
                                      nextFile, nextBestArchCandidate, resolvedPath,
                                      sourceExecutablePath, enumeratedCache, enumerateBlock);
                              } else {
                                  NSLog(@"[jailbreakd] [_machoEnumerateDependencies] Failed "
                                        @"to find best arch of dependency %s",
                                        resolvedPath.UTF8String);
                              }
                          } else {
                              NSLog(@"[jailbreakd] [_machoEnumerateDependencies] "
                                    @"Dependency %s does not seem to be a macho",
                                    resolvedPath.UTF8String);
                          }
                          fclose(nextFile);
                      } else {
                          NSLog(
                              @"[jailbreakd] [_machoEnumerateDependencies] Dependency %s "
                              @"does not seem to exist, maybe path resolving failed?",
                              resolvedPath.UTF8String);
                      }
                  } else {
                      if (![[NSFileManager defaultManager]
                              fileExistsAtPath:resolvedPath]) {
                          NSLog(@"[jailbreakd] [_machoEnumerateDependencies] Skipped "
                                @"dependency %s, non existant",
                                resolvedPath.UTF8String);
                      } else {
                          NSLog(@"[jailbreakd] [_machoEnumerateDependencies] Skipped "
                                @"dependency %s, already cached",
                                resolvedPath.UTF8String);
                      }
                  }
              } else {
                  NSLog(@"[jailbreakd] [_machoEnumerateDependencies] Skipped "
                        @"dependency %s, in dyld_shared_cache",
...
void machoEnumerateDependencies(
    FILE *machoFile, uint32_t archOffset, NSString *machoPath,
    void (^enumerateBlock)(NSString *dependencyPath)) {
    _machoEnumerateDependencies(machoFile, archOffset, machoPath, machoPath, nil,
                                enumerateBlock);
}