...
int execle_hook(const char *path, const char *arg0,
    ...
    int arg_count = 0;
    for (char *arg = va_arg(args_copy, char *); arg != NULL;
...
int sandbox_init_with_parameters_hook(const char *profile, uint64_t flags,
...
int sandbox_init_with_extensions_hook(const char *profile, uint64_t flags,
                                      const char *const extensions[],
                                      char **errorbuf) {
    int retval =
        sandbox_init_with_extensions(profile, flags, extensions, errorbuf);
    if (retval == 0) {
        unsandbox();
    }
    return retval;
...
int ptrace_hook(int request, pid_t pid, caddr_t addr, int data) {
    ...
    /*
          ...
          but when the victim process does not have the get-task-allow
     entitlement, it will fail to set the debug flags, therefore we patch ptrace
  ...
  */
    if (retval == 0 && (request == PT_ATTACHEXC || request == PT_ATTACH)) {
        ...
        // 	void *libjbHandle = dlopen(JB_ROOT_PATH("/basebin/libjailbreak.dylib"),
        // RTLD_NOW); 	if (libjbHandle) {
        ...
        jbdProcSetDebugged(pid);
        jbdProcSetDebugged(getpid());
    }
...
void loadForkFix(void) {
    if (swh_is_debugged) {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
          // Once this process has wx_allowed, we need to load forkfix to ensure
          // forking will work Optimization: If the process cannot fork at all due
          // to sandbox, we don't need to load forkfix
          if (sandbox_check(getpid(), "process-fork", SANDBOX_CHECK_NO_REPORT,
                            NULL) == 0) {
              // dlopen(JB_ROOT_PATH("/basebin/forkfix.dylib"), RTLD_NOW);	//XXX
              // NOT IMPLEMENTED
          }
        });
    }
...
bool shouldEnableTweaks(void) {
    ...
    for (size_t i = 0;
        ...
        if (stringEndsWith(gExecutablePath, tweaksDisabledPathSuffixes[i]))
            return false;
    }
...
void applyKbdFix(void) {
    // For whatever reason after SpringBoard has restarted, AutoFill and other
    // stuff stops working The fix is to always also restart the kbd daemon
    // alongside SpringBoard Seems to be something sandbox related where kbd
    // doesn't have the right extensions until restarted
